...
newtype PersistentBackendT m a = PersistentBackendT { unPersistentBackendT :: PersistentBackendInternal m a }
  deriving (Functor, Applicative, Monad, MonadIO, MonadCont, MonadError ServerError)

type PersistentBackendInternal m = ReaderT (AuthConfig, ConnectionPool) (ExceptT ServerError (SqlPersistT m))
...
mapPersistentBackendT :: (m (Either ServerError a) -> n (Either ServerError b))
...
runPersistentBackendT :: (MonadUnliftIO m, MonadBaseControl IO m) => AuthConfig -> ConnectionPool -> PersistentBackendT m a -> m (Either ServerError a)
