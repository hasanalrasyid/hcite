...
makeAppConfig = do
  ...
  c <- BaseApp.makeContext (KL.InitialLogNamespace "dev" "hciteservice") prometheusEnv versions grpcConfig
...
makeKatipScribe kcfg LogLevel{..} le = case kcfg of
  ...
  ES {host, port} -> do
    ...
    esScribe <- ES.mkEsScribe ES.defaultEsScribeCfgV5 bloodhoundEnv (BH.IndexName "hciteservice")
